...
namespace Thrift.Protocol
{
   ...
   class TJsonProtocol : TProtocol
   {
      ...
      {
         byte ch = await Reader.ReadAsync(cancellationToken);
         if (ch != bytes[0])
         {
      ...
      {
         byte[] b = Utf8Encoding.GetBytes(s);
         await WriteJsonStringAsync(b, cancellationToken);
      }
      ...
      private async ValueTask<byte[]> ReadJsonStringAsync(bool skipContext, CancellationToken cancellationToken)
      {
         using (var buffer = new MemoryStream())
         {
            ...
            while (true)
            {
               ...
               {
                  int off = Array.IndexOf(TJSONProtocolConstants.EscapeChars, (char)ch);
                  if (off == -1)
                  {
               ...
               short wch = (short)((TJSONProtocolHelper.ToHexVal(_tempBuffer[0]) << 12) +
               ...
               {
                  byte[] tmp = Utf8Encoding.GetBytes(new[] { (char)wch });
                  await buffer.WriteAsync(tmp, 0, tmp.Length, cancellationToken);
               }
            }
      ...
      private async ValueTask<string> ReadJsonNumericCharsAsync(CancellationToken cancellationToken)
      {
         ...
         while (true)
         {
            ...
            try
            {
               byte ch = await Reader.PeekAsync(cancellationToken);
               if (!TJSONProtocolHelper.IsJsonNumeric(ch))
               {
                  break;
               }
               char c = (char)await Reader.ReadAsync(cancellationToken);
               strbld.Append(c);
            }
      ...
      private async ValueTask<long> ReadJsonIntegerAsync(CancellationToken cancellationToken)
      {
         ...
         string str = await ReadJsonNumericCharsAsync(cancellationToken);
         if (Context.EscapeNumbers())
         {
            await ReadJsonSyntaxCharAsync(TJSONProtocolConstants.Quote, cancellationToken);
         }
      ...
      private async ValueTask<double> ReadJsonDoubleAsync(CancellationToken cancellationToken)
      {
         ...
         {
            byte[] arr = await ReadJsonStringAsync(true, cancellationToken);
      ...
      {
         byte[] b = await ReadJsonStringAsync(false, cancellationToken);
         int len = b.Length;
         int off = 0;
         int size = 0;
      ...
      public override async ValueTask<TMessage> ReadMessageBeginAsync(CancellationToken cancellationToken)
      {
         ...
         byte[] buf = await ReadJsonStringAsync(false, cancellationToken);
