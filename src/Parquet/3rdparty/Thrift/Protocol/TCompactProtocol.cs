...
namespace Thrift.Protocol
{
   ...
   class TCompactProtocol : TProtocol
   {
      ...
      private async Task WriteFieldBeginInternalAsync(TField field, byte fieldType, CancellationToken cancellationToken)
      {
         ...
         {
            int delta = field.ID - _lastFieldId;
            if (delta <= 15)
            {
               // Write them together
      ...
      public override async Task WriteBoolAsync(bool b, CancellationToken cancellationToken)
      {
         ...
         */
         ...
         if (_booleanField != null)
         {
            ...
            byte type = b ? Types.BooleanTrue : Types.BooleanFalse;
      ...
      public override async Task WriteStringAsync(string str, CancellationToken cancellationToken)
      {
         ...
         byte[] buf = ArrayPool<byte>.Shared.Rent(Encoding.UTF8.GetByteCount(str));
         try
         {
            int numberOfBytes = Encoding.UTF8.GetBytes(str, 0, str.Length, buf, 0);
      ...
      public override async ValueTask<TMessage> ReadMessageBeginAsync(CancellationToken cancellationToken)
      {
         ...
         byte protocolId = (byte)await ReadByteAsync(cancellationToken);
         if (protocolId != ProtocolId)
         {
            throw new TProtocolException($"Expected protocol id {ProtocolId:X} but got {protocolId:X}");
         }

         byte versionAndType = (byte)await ReadByteAsync(cancellationToken);
         byte version = (byte)(versionAndType & VersionMask);
         ...
         byte type = (byte)((versionAndType >> TypeShiftAmount) & TypeBits);
         int seqid = (int)await ReadVarInt32Async(cancellationToken);
         string messageName = await ReadStringAsync(cancellationToken);
      ...
      public override async ValueTask<TField> ReadFieldBeginAsync(CancellationToken cancellationToken)
      {
         ...
         byte type = (byte)await ReadByteAsync(cancellationToken);
         ...
         byte compactType = (byte)(type & 0x0f);
         ...
         TType ttype = GetTType(compactType);
      ...
      public override async ValueTask<TMap> ReadMapBeginAsync(CancellationToken cancellationToken)
      {
         ...
         */
         ...
         int size = (int)await ReadVarInt32Async(cancellationToken);
         byte keyAndValueType = size == 0 ? (byte)0 : (byte)await ReadByteAsync(cancellationToken);
      ...
      public override ValueTask<bool> ReadBoolAsync(CancellationToken cancellationToken)
      {
         ...
         */
         ...
         {
            sbyte data = await ReadByteAsync(cancellationToken);
      ...
      {
         // read length
         int length = (int)await ReadVarInt32Async(cancellationToken);
         if (length == 0)
         {
            return string.Empty;
         }
      ...
      public override async ValueTask<TList> ReadListBeginAsync(CancellationToken cancellationToken)
      {
         ...
         */
         ...
         byte sizeAndType = (byte)await ReadByteAsync(cancellationToken);
         ...
         TType type = GetTType(sizeAndType);
      ...
      private async ValueTask<uint> ReadVarInt32Async(CancellationToken cancellationToken)
      {
         ...
         */
         ...
         while (true)
         {
            byte b = (byte)await ReadByteAsync(cancellationToken);
